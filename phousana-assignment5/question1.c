...
int* insert(const int anArray[], int arrLen, int position, int value) {
    ...
    int *array = malloc(sizeof(int) * (arrLen+1));
...
int* erase(const int anArray[], int arrLen, int position) {
  ...
  // Fill in this function!
     int *array = malloc((arrLen) * sizeof(int)+1);
      ...
      array[position] = 0;
      for(int i=position; i<arrLen-1; i++){
          array[i] = array[i+1]; 
      }
      return(array);  
}
